...
void loadImages(std::vector<cv::Mat> &lefts, std::vector<cv::Mat> &rights,
        const int &fileNum) {
    cv::namedWindow("Left", cv::WINDOW_AUTOSIZE | cv::WINDOW_FREERATIO);
    cv::namedWindow("Right", cv::WINDOW_AUTOSIZE | cv::WINDOW_FREERATIO);
...
int findChessboards(
        std::vector<cv::Mat> &lefts, std::vector<cv::Mat> &rights,
